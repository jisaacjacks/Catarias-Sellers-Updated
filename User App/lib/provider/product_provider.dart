...
class ProductProvider extends ChangeNotifier {
  ...
  //latest product
  Future<void> getLatestProductList(int offset, BuildContext context, String languageCode, {bool reload = false}) async {
    if(reload) {
      _offsetList = [];
      _latestProductList = [];
    }
    ...
    if(!_offsetList.contains(offset)) {
      ...
      ApiResponse apiResponse = await productRepo.getLatestProductList(context,offset.toString(), languageCode, productType, title);
  ...
  Future<int> getLatestOffset(BuildContext context, String languageCode) async {
    ApiResponse apiResponse = await productRepo.getLatestProductList(context,'1', languageCode,productType,title);
 ...
 void changeTypeOfProduct(ProductType type, String title){
  ...
  void initSellerProductList(String sellerId, int offset, BuildContext context, String languageCode,  {bool reload = false}) async {
    ...
    ApiResponse apiResponse = await productRepo.getSellerProductList(sellerId, offset.toString(), languageCode);
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
